...
const ID = { fid: FID, network: FarcasterNetwork.MAINNET };
function signer(){ const hex = JSON.parse(fs.readFileSync(H + '/.openclaw/farcaster-credentials.json', 'utf8')).signerPrivateKey; return new NobleEd25519Signer(Buffer.from(hex, 'hex')); }
...
async function submit(msg){ const res = await fetch(HUB + '/v1/submitMessage', { method: 'POST', headers: { 'Content-Type': 'application/octet-stream', 'x-api-key': KEY }, body: Buffer.from(Message.encode(msg).finish()) }); if (!res.ok) throw new Error('hub ' + res.status + ' ' + (await res.text()).slice(0, 160)); return res; }
async function post({ text, embedUrl, mentions = [], mentionsPositions = [], parentFid, parentHash, parentUrl }){
  assertPostingEnabled('post');
  const body = { text: clean(text).slice(0, 320), embeds: embedUrl ? [{ url: embedUrl }] : [], embedsDeprecated: [], mentions, mentionsPositions, type: CastType.CAST };
  if (parentFid && parentHash) body.parentCastId = { fid: parentFid, hash: hbuf(parentHash) };
  if (parentUrl) body.parentUrl = parentUrl;
  const m = await makeCastAdd(body, ID, signer()); if (m.isErr()) throw new Error(m.error.message);
  await submit(m.value); return m.value.hash;
}
async function remove(hash){ assertPostingEnabled('remove'); const m = await makeCastRemove({ targetHash: hbuf(hash) }, ID, signer()); if (m.isErr()) throw new Error(m.error.message); await submit(m.value); }
async function follow(targetFid){ assertPostingEnabled('follow'); const m = await makeLinkAdd({ type: 'follow', targetFid }, ID, signer()); if (m.isErr()) throw new Error(m.error.message); await submit(m.value); return m.value.hash; }
...
async function quoteCast({ text, quotedFid, quotedHash, embedUrl }) {
  ...
  const m = await makeCastAdd({ text: clean(text).slice(0, 320), embeds, embedsDeprecated: [], mentions: [], mentionsPositions: [], type: CastType.CAST }, ID, signer());
